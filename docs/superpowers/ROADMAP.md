...
- ✅ **A4.a (High) — exact-match role allowlists** — closed by PR #PRNUM. Replaced regex matching with exact-address membership + a READER-only `"*"` sentinel, validated at startup (`Role.validate_config` → `InvalidRoleConfigError`). Test: `test_a4_a_overbroad_admin_regex_does_not_escalate` (flipped from xfail to passing regression) + new exact-match/validation coverage in `tests/test_api.py`.

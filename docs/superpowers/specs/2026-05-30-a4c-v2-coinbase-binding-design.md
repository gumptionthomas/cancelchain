...
- **Coinbases are block-specific; cross-fork coinbase replay is correctly rejected.** v1's "cross-fork coinbase replay is legitimate" premise (extending audit Attack b) was wrong for coinbases. A coinbase is the reward minted by exactly one block-position; binding it to its parent makes reusing it elsewhere invalid. Attack b's legitimacy applies to *regular* txns (a spend valid on competing forks), not coinbases. The v1 cross-fork-acceptance test is removed.

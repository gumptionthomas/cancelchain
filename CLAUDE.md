...
Key `CC_*` settings: `NODE_HOST`, `PEERS` (list of `http(s)://<peer-address>@host` URLs — username component is the peer's wallet address), `WALLET_DIR`, `DEFAULT_COMMAND_HOST`, `{ADMIN,MILLER,TRANSACTOR,READER}_ADDRESSES` (exact-address allowlists matched against the authenticated address in `api.Role.address_role`; `READER_ADDRESSES` may contain the literal `"*"` to grant READER to any authenticated wallet; a non-address entry, or `"*"` outside `READER_ADDRESSES`, is rejected at startup via `Role.validate_config` → `InvalidRoleConfigError`). `WALLET_DIR` is walked at startup; every `*.pem` becomes an in-memory `Wallet` in `app.wallets`, keyed by address.
